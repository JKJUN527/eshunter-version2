...
//        return $data;
        return view('mobile/my/myApplyRecord', ['data' => $data]);
    }
...
//        return $data;
        return view('mobile.my.deliverList', ['data' => $data]);
    }
    //企业删除收到的投递记录（清空或删除所有）
    //传入did
...
//            return $data;
            return view('mobile/position/deliverDetail', ['data' => $data]);
        } else {
            return redirect()->back();
        }
    }
...
//        return $data;
        return view('mobile/position/publishPosition', ['data' => $data]);
    }
...
//        }
        ...
        //return $data;
        return view('mobile.position.publishList', ['data' => $data]);
        //return $position;
    }
    ...
    public function searchPosition(Request $request) {
        ...
        if ($uid == 0) return view("mobile.my.login", ['data' => $data]);
    ...
    public function delPosition(Request $request) {
        ...
        $type = AuthController::getType();
        if ($uid == 0 || $type != 2) {
            return view('mobile.my.login')->with('error', '请登录后操作');
        }
        $pid = $request->input('pid');
        $position = Position::find($pid);
...
//         return $data;
        return view('mobile/job/jobView', ["data" => $data]);
    }
