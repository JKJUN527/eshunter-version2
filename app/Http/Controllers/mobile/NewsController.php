...
class NewsController extends Controller {
    public function index() {
        ...
        return view('mobile/information/information');
    }
...
//        return $data;
        return view('mobile/information/infoDetail', ['data' => $data]);
    }
    ...
    public function LoadMore(Request $request){
        ...
        $data['status'] = 400;
        $data['msg'] = "参数错误";
        if($request->has('type') &&$request->has('currentNum')&&$request->has('needNum')){
            $type = $request->input('type');
            $currentNum = $request->input('currentNum');
            $needNum = $request->input('needNum');
            $data['news'] = News::where('type',$type)
    ...
    public function SearchNews(Request $request, $pagnum = 9) {
        ...
        if($request->has('pagnum')){
            $pagnum = $request->input('pagnum');
        }
        $data['newtype'] = $type;
...
//        return view('news.index', ['data' => $data]);
        return view('mobile/information/information', ['data' => $data]);
    }
